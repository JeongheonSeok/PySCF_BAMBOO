...
def ptdict_to_pkldict(ptdir):
    ...
    for atom in pkldict['atom_types']:
        energy -= 2*ENERGY_ATOM[atom]
    pkldict['energy'] = energy
    
    return pkldict

def mk_pkl_from_pt(filedir='./', outdir='./fixed_pkls'):
    pkl_files = glob.glob(os.path.join(filedir, '*.pkl'))
    for old_pkl_dir in tqdm(pkl_files):
        pkldict = fix_pkldict(old_pkl_dir)
