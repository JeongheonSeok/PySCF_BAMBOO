...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 19 17:00:00 2024
...
def zip_pt_files(directory, output_filename='pt.zip'):
        ...
        for foldername, subfolders, filenames in os.walk(directory):
            for filename in tqdm(filenames, desc="Make .zip file", unit="files"):
                if filename.endswith('.pt'):
                    pt_file_path = os.path.join(foldername, filename)
                    #xyz_filename = filename.replace('.pt', '.xyz')
                    #xyz_file_path = os.path.join(foldername, xyz_filename)

                    zipf.write(pt_file_path, os.path.relpath(pt_file_path, directory))
